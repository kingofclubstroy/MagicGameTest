...
namespace NativeQuadTree
{
    public unsafe partial struct NativeQuadTree 
    {
        struct QuadTreeRangeQuery
        {
            ...
            public void Query(NativeQuadTree tree, AABB2D bounds, NativeList<ObstacleStruct> results)
            {
                this.tree = tree;
                this.bounds = bounds;
                count = 0;
            ...
            public void RecursiveRangeQuery(AABB2D parentBounds, bool parentContained, int prevOffset, int depth)
            {
                ...
                {
                    fastResults->Resize<ObstacleStruct>(math.max(fastResults->Capacity * 2, count + 4 * tree.maxLeafElements));
                }
                ...
                for (int l = 0; l < 4; l++)
                {
                    ...
                    else if (elementCount != 0)
                    {
                        ...
                        if (contained)
                        {
                            var index = (void*)((IntPtr)tree.elements->Ptr + node.firstChildIndex * UnsafeUtility.SizeOf<ObstacleStruct>());

                            UnsafeUtility.MemCpy((void*)((IntPtr)fastResults->Ptr + count * UnsafeUtility.SizeOf<ObstacleStruct>()),
                                index, node.count * UnsafeUtility.SizeOf<ObstacleStruct>());
                            count += node.count;
                        }
                        else
                        {
                            ...
                            {
                                var element = UnsafeUtility.ReadArrayElement<ObstacleStruct>(tree.elements->Ptr, node.firstChildIndex + k);
