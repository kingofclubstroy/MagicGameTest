...
namespace NativeQuadTree
{
    ...
    public unsafe partial struct NativeQuadTree : IDisposable
    {
        ...
        ) : this()
        {
            ...
            elements = UnsafeList.Create(UnsafeUtility.SizeOf<ObstacleStruct>(),
                UnsafeUtility.AlignOf<ObstacleStruct>(),
                initialElementsCapacity,
                allocator);
        }

        public void ClearAndBulkInsert(NativeArray<ObstacleStruct> incomingElements)
        {
            ...
            {
                elements->Resize<ObstacleStruct>(math.max(incomingElements.Length, elements->Capacity * 2));
            }
            ...
            {
                var incPos = incomingElements[i].position;
        ...
        public void RangeQuery(AABB2D bounds, NativeList<ObstacleStruct> results)
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
        ...
        public void Clear()
        {
            ...
            UnsafeUtility.MemClear(elements->Ptr, elements->Capacity * UnsafeUtility.SizeOf<ObstacleStruct>());
            elementsCount = 0;
        }
